...
import pk.elfo.gameserver.network.clientpackets.AbstractRefinePacket;
...
public class AugmentationData
{
	...
	protected AugmentationData()
	{
		...
		_log.info(getClass().getSimpleName() + ": " + (_augStats.get(0).size() * 4) + " augmentation stats.");
		_log.info(getClass().getSimpleName() + ": " + (_augAccStats.get(0).size() * 4) + " accessory augmentation stats.");
		for (int i = 0; i < 10; i++)
		{
			_log.info(getClass().getSimpleName() + ": " + _blueSkills.get(i).size() + " blue, " + _purpleSkills.get(i).size() + " purple e " + _redSkills.get(i).size() + " red skills para lifeStoneLevel " + i);
		}
	}
