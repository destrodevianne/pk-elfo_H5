...
public abstract class L2Skill implements IChanceSkillTrigger
{
	...
	private final boolean _ignoreDefence;
	...
	protected L2Skill(StatsSet set)
	{
		...
		_ignoreDefence = set.getBool("ignoreDefence", false);
	...
	public static final boolean checkForAreaOffensiveSkills(L2Character caster, L2Character target, L2Skill skill, boolean sourceInArena)
	{
		...
		if (player != null)
		{
			if (targetPlayer != null)
			{
				...
				if (player.isInParty() && targetPlayer.isInParty() && !player.allowPeaceAttack())
				{
					// Same party
				...
				if (EventsInterface.isParticipating(player.getObjectId()) && EventsInterface.isParticipating(targetPlayer.getObjectId()))
				{
					if (EventsInterface.areTeammates(player.getObjectId(), targetPlayer.getObjectId()))
					{
						return false;
					}
				}
