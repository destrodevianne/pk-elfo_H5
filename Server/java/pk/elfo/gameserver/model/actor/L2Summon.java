...
import pk.elfo.gameserver.model.actor.L2Attackable.AggroInfo;
...
import pk.elfo.gameserver.model.actor.knownlist.SummonKnownList;
...
import pk.elfo.gameserver.network.serverpackets.ExPartyPetWindowAdd;
import pk.elfo.gameserver.network.serverpackets.ExPartyPetWindowDelete;
import pk.elfo.gameserver.network.serverpackets.ExPartyPetWindowUpdate;
import pk.elfo.gameserver.network.serverpackets.L2GameServerPacket;
import pk.elfo.gameserver.network.serverpackets.PetDelete;
import pk.elfo.gameserver.network.serverpackets.PetInfo;
import pk.elfo.gameserver.network.serverpackets.PetItemList;
import pk.elfo.gameserver.network.serverpackets.PetStatusUpdate;
...
import pk.elfo.gameserver.network.serverpackets.AbstractNpcInfo.SummonInfo;
...
public abstract class L2Summon extends L2Playable
{
	...
	public void sendDamageMessage(L2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss)
	{
		...
		if (target.getObjectId() != getOwner().getObjectId())
		{
			...
			if (getOwner().isInOlympiadMode() && (target instanceof L2PcInstance) && ((L2PcInstance) target).isInOlympiadMode() && (((L2PcInstance) target).getOlympiadGameId() == getOwner().getOlympiadGameId()) && !((L2PcInstance) target).isInvul() && !((L2PcInstance) target).inObserverMode())
			{
	...
	public void updateAndBroadcastStatus(int val)
	{
		...
		sendPacket(new SummonInfo(this, this.getOwner(), val));
		if (isVisible())
		{
