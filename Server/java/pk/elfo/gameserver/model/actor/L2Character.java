...
import pk.elfo.gameserver.ai.L2AttackableAI;
...
import pk.elfo.gameserver.model.ChanceSkillList;
import pk.elfo.gameserver.model.CharEffectList;
import pk.elfo.gameserver.model.FusionSkill;
...
import pk.elfo.gameserver.model.actor.instance.L2NpcWalkerInstance;
...
import pk.elfo.gameserver.model.actor.instance.L2PcInstance.SkillDat;
import pk.elfo.gameserver.model.actor.knownlist.CharKnownList;
import pk.elfo.gameserver.model.actor.position.CharPosition;
import pk.elfo.gameserver.model.actor.stat.CharStat;
import pk.elfo.gameserver.model.actor.status.CharStatus;
...
import pk.elfo.gameserver.model.interfaces.IChanceSkillTrigger;
import pk.elfo.gameserver.model.interfaces.ISkillsHolder;
...
import pk.elfo.gameserver.model.stats.Calculator;
...
import pk.elfo.gameserver.network.serverpackets.Attack;
import pk.elfo.gameserver.network.serverpackets.ChangeMoveType;
...
import pk.elfo.gameserver.network.serverpackets.MagicSkillCanceld;
...
import pk.elfo.gameserver.network.serverpackets.MoveToLocation;
import pk.elfo.gameserver.network.serverpackets.Revive;
import pk.elfo.gameserver.network.serverpackets.ServerObjectInfo;
...
import pk.elfo.gameserver.network.serverpackets.TeleportToLocation;
...
import pk.elfo.gameserver.util.L2TIntObjectHashMap;
...
public abstract class L2Character extends L2Object implements ISkillsHolder
{
	...
	private boolean _protected = false; // Buff protection
	...
	{
		for (L2WorldRegion neighbor : getWorldRegion().getSurroundingRegions())
		{
			for (L2Playable playable : neighbor.getVisiblePlayable().values())
			{
				if (playable instanceof L2PcInstance)
				{
					((L2PcInstance) playable).sendPacket(mov);
				}
			}
		}
	}
	...
	protected void doAttack(L2Character target)
	{
		...
		if ((this instanceof L2Summon) && EventsInterface.doAttack(((L2Summon) this).getOwner().getObjectId(), target.getObjectId()))
		{
			sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
	...
	private void beginCast(L2Skill skill, boolean simultaneously)
	{
		if (this instanceof L2PcInstance)
		{
			if (EventsInterface.isParticipating(getObjectId()))
			{
				...
				if (getTarget() != null)
				{
					if (!EventsInterface.canAttack(getObjectId(), getTarget().getObjectId()) && skill.isOffensive())
					{
						if (simultaneously)
						{
							setIsCastingSimultaneouslyNow(false);
		...
		if (this instanceof L2Summon)
		{
			if (EventsInterface.isParticipating(((L2Summon) this).getOwner().getObjectId()))
			{
				...
				if (!EventsInterface.onUseMagic(((L2Summon) this).getOwner().getObjectId(), skill.getId()))
				{
					if (simultaneously)
					{
						setIsCastingSimultaneouslyNow(false);
	...
	public boolean doDie(L2Character killer)
	{
		...
		else if (!isInTownWarEvent() || Config.TW_LOSE_BUFFS_ON_DEATH || Config.LEAVE_BUFFS_ON_DIE)
		{
		...
		if ((targets.length > 0) && (escapeRange > 0))
		{
			...
			for (L2Object target : targets)
			{
				if (target instanceof L2Character)
				{
					...
					if ((skill.getTargetType() != L2TargetType.PARTY || !skill.hasEffectType(L2EffectType.HEAL)) && (Config.GEODATA > 0) && !GeoData.getInstance().canSeeTarget(this, target))
					{
						_skipgeo++;
						continue;
					}
