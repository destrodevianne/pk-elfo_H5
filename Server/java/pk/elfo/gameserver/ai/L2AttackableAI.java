...
import pk.elfo.gameserver.model.actor.instance.L2FestivalMonsterInstance;
import pk.elfo.gameserver.model.actor.instance.L2FriendlyMobInstance;
import pk.elfo.gameserver.model.actor.instance.L2GrandBossInstance;
import pk.elfo.gameserver.model.actor.instance.L2GuardInstance;
...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private boolean autoAttackCondition(L2Character target)
	{
		...
		if (target.isPlayable())
		{
			...
			if (!(me.isRaid()) && !(me.canSeeThroughSilentMove()) && !(me instanceof L2GuardInstance) && ((L2Playable) target).isSilentMoving())
			{
				return false;
			}
		}
	...
	protected void onIntentionAttack(L2Character target)
	{
		...
		if ((target instanceof L2PcInstance) && (((L2PcInstance) target).getKarma() > 0) && !((L2PcInstance) target).isCursedWeaponEquipped())
		{
			super.onIntentionAttack(target);
		}
		else if (!(getActiveChar() instanceof L2GuardInstance) || ((getActiveChar() instanceof L2GuardInstance) && (getActiveChar().getNpcId() != Config.GUARD_ID)))
		{
			super.onIntentionAttack(target);
		}
