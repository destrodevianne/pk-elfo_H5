package pk.elfo.gameserver.scripts.zone;
...
public final class DragonValley extends AbstractNpcAI
{
    ...
    private void manageMoraleBoost(L2PcInstance player, L2Npc npc)
    {
        ...
        if (player.isInParty() && (player.getParty().getMemberCount() >= MIN_MEMBERS) && (npc != null))
        {
            ...
            {
                if ((member.getLevel() >= MIN_LVL) && (member.getClassId().level() >= CLASS_LVL) && (Util.calculateDistance(npc, member, true) < MIN_DISTANCE))
                {
            ...
            for (L2PcInstance member : player.getParty().getMembers())
            {
                if (Util.calculateDistance(npc, member, true) < MIN_DISTANCE)
                {
