...
public final class SpawnTable
{
	...
	public void load()
	{
		if (!Config.ALT_DEV_NO_SPAWNS)
		{
			...
			_log.info(getClass().getSimpleName() + ": " + spawnCount + " npc spawns.");
			if (Config.CUSTOM_SPAWNLIST_TABLE)
			{
				fillSpawnTable(true);
				_log.info(getClass().getSimpleName() + ": " + (_spawnTable.size() - spawnCount) + " custom npc spawns.");
			}
		}
	}
	...
	private int fillSpawnTable(boolean isCustom)
	{
			...
			ResultSet rs = s.executeQuery(isCustom ? SELECT_CUSTOM_SPAWNS : SELECT_SPAWNS))
		{
			...
			while (rs.next())
			{
				...
				if (npcTemplate.isType("L2SiegeGuard") || npcTemplate.isType("L2RaidBoss") || (!Config.ALLOW_CLASS_MASTERS && npcTemplate.isType("L2ClassMaster")) || (!Config.ALLOW_HITMAN_GDE && npcTemplate.isType("L2Hitman")))
				{
					// Don't spawn
					continue;
				}
