...
import king.server.gameserver.ai.L2PlayerAI;
import king.server.gameserver.ai.L2SummonAI;
import king.server.gameserver.cache.HtmCache;
import king.server.gameserver.cache.WarehouseCacheManager;
...
import king.server.gameserver.datatables.SkillTable.FrequentSkill;
...
import king.server.gameserver.handler.ItemHandler;
...
import king.server.gameserver.model.L2PremiumItem;
import king.server.gameserver.model.L2Radar;
import king.server.gameserver.model.L2RecipeList;
import king.server.gameserver.model.L2Request;
...
import king.server.gameserver.model.actor.knownlist.PcKnownList;
import king.server.gameserver.model.actor.position.PcPosition;
...
import king.server.gameserver.model.base.ClassLevel;
import king.server.gameserver.model.base.PlayerClass;
import king.server.gameserver.model.base.Race;
import king.server.gameserver.model.base.Sex;
import king.server.gameserver.model.base.SubClass;
...
import king.server.gameserver.model.itemcontainer.PcRefund;
import king.server.gameserver.model.itemcontainer.PcWarehouse;
import king.server.gameserver.model.itemcontainer.PetInventory;
...
import king.server.gameserver.model.stats.Rates;
...
import king.server.gameserver.network.serverpackets.ChangeWaitType;
...
import king.server.gameserver.network.serverpackets.ExAutoSoulShot;
...
import king.server.gameserver.network.serverpackets.ExDominionWarStart;
import king.server.gameserver.network.serverpackets.ExDuelUpdateUserInfo;
import king.server.gameserver.network.serverpackets.ExFishingEnd;
import king.server.gameserver.network.serverpackets.ExFishingStart;
...
import king.server.gameserver.network.serverpackets.ExOlympiadMode;
import king.server.gameserver.network.serverpackets.ExPrivateStoreSetWholeMsg;
import king.server.gameserver.network.serverpackets.ExRedSky;
import king.server.gameserver.network.serverpackets.ExSetCompassZoneCode;
import king.server.gameserver.network.serverpackets.ExSpawnEmitter;
import king.server.gameserver.network.serverpackets.ExStartScenePlayer;
...
import king.server.gameserver.network.serverpackets.LeaveWorld;
...
import king.server.gameserver.network.serverpackets.ObservationMode;
import king.server.gameserver.network.serverpackets.ObservationReturn;
import king.server.gameserver.network.serverpackets.PartySmallWindowUpdate;
import king.server.gameserver.network.serverpackets.PetInventoryUpdate;
import king.server.gameserver.network.serverpackets.PlaySound;
import king.server.gameserver.network.serverpackets.PledgeShowMemberListDelete;
import king.server.gameserver.network.serverpackets.PledgeShowMemberListUpdate;
import king.server.gameserver.network.serverpackets.PrivateStoreListBuy;
import king.server.gameserver.network.serverpackets.PrivateStoreListSell;
import king.server.gameserver.network.serverpackets.PrivateStoreManageListBuy;
import king.server.gameserver.network.serverpackets.PrivateStoreManageListSell;
import king.server.gameserver.network.serverpackets.PrivateStoreMsgBuy;
import king.server.gameserver.network.serverpackets.PrivateStoreMsgSell;
import king.server.gameserver.network.serverpackets.RecipeShopMsg;
import king.server.gameserver.network.serverpackets.RecipeShopSellList;
import king.server.gameserver.network.serverpackets.RelationChanged;
import king.server.gameserver.network.serverpackets.Ride;
...
import king.server.gameserver.network.serverpackets.ShortBuffStatusUpdate;
...
import king.server.gameserver.network.serverpackets.SkillList;
import king.server.gameserver.network.serverpackets.Snoop;
...
import king.server.gameserver.network.serverpackets.StopMove;
import king.server.gameserver.network.serverpackets.SystemMessage;
import king.server.gameserver.network.serverpackets.TargetSelected;
import king.server.gameserver.network.serverpackets.TargetUnselected;
import king.server.gameserver.network.serverpackets.TradeDone;
import king.server.gameserver.network.serverpackets.TradeOtherDone;
import king.server.gameserver.network.serverpackets.TradeStart;
...
import king.server.gameserver.util.PlayerEventStatus;
...
public final class L2PcInstance extends L2Playable
{
	...
	private static final String INSERT_CHARACTER = "INSERT INTO characters (account_name,charId,char_name,level,maxHp,curHp,maxCp,curCp,maxMp,curMp,face,hairStyle,hairColor,sex,exp,sp,karma,fame,pvpkills,pkkills,clanid,race,classid,deletetime,cancraft,title,title_color,accesslevel,online,isin7sdungeon,clan_privs,wantspeace,base_class,newbie,nobless,power_grade,createDate,aio,aio_end,vip,vip_end) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	private static final String UPDATE_CHARACTER = "UPDATE characters SET level=?,maxHp=?,curHp=?,maxCp=?,curCp=?,maxMp=?,curMp=?,face=?,hairStyle=?,hairColor=?,sex=?,heading=?,x=?,y=?,z=?,exp=?,expBeforeDeath=?,sp=?,karma=?,fame=?,pvpkills=?,pkkills=?,clanid=?,race=?,classid=?,deletetime=?,title=?,title_color=?,accesslevel=?,online=?,isin7sdungeon=?,clan_privs=?,wantspeace=?,base_class=?,onlinetime=?,punish_level=?,punish_timer=?,newbie=?,nobless=?,power_grade=?,subpledge=?,lvl_joined_academy=?,apprentice=?,sponsor=?,varka_ketra_ally=?,clan_join_expiry_time=?,clan_create_expiry_time=?,char_name=?,death_penalty_level=?,bookmarkslot=?,vitality_points=?,pccafe_points=?,language=?,game_points=?,hitman_target=?,aio=?,aio_end=?,vip=?,vip_end=? WHERE charId=?";
	private static final String RESTORE_CHARACTER = "SELECT account_name, charId, char_name, level, maxHp, curHp, maxCp, curCp, maxMp, curMp, face, hairStyle, hairColor, sex, heading, x, y, z, exp, expBeforeDeath, sp, karma, fame, pvpkills, pkkills, clanid, race, classid, deletetime, cancraft, title, title_color, accesslevel, online, char_slot, lastAccess, clan_privs, wantspeace, base_class, onlinetime, isin7sdungeon, punish_level, punish_timer, newbie, nobless, power_grade, subpledge, lvl_joined_academy, apprentice, sponsor, varka_ketra_ally,clan_join_expiry_time,clan_create_expiry_time,death_penalty_level,bookmarkslot,vitality_points,pccafe_points,createDate,language,game_points,hitman_target,aio,aio_end,vip,vip_end FROM characters WHERE charId=?";
	...
	public void setTarget(L2Object newTarget)
	{
		...
		if (newTarget instanceof L2PcInstance)
		{
			...
			if (!EventsInterface.canTargetPlayer(getObjectId(), player.getObjectId()))
			{
				return;
			}
		}
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (killer != null)
		{
			...
			if (pk != null)
			{
				if (EventsInterface.isParticipating(getObjectId()) && EventsInterface.isParticipating(pk.getObjectId()))
				{
					EventsInterface.onKill(getObjectId(), pk.getObjectId());
					EventsInterface.onDie(getObjectId(), pk.getObjectId());
				}
			}
   ...
   //added Efeito Vitality no peitoral de algumas armaduras
   public void checkForVitalityEffect()
   {
       ...
       {
           _log.log(Level.WARNING,"Mensagem de excecao de Armadura: " + e.getMessage(),e);
   ...
   // Fim do codigo de Efeito Vitality no peitoral de algumas armaduras
   
   /**
	 * Restaurar dados para este livro de receitas L2PcInstance.
	 * @param loadCommon
	 */
	...
	private void storeCharBase()
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			...
			statement.setInt(55, getHitmanTarget());
			statement.setInt(56, isAio() ? 1 : 0);
			statement.setLong(57, getAioEndTime());
	...
	private boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		if (skill.getSkillType() == L2SkillType.INSTANT_JUMP)
		{
			...
			if (isInsideZone(ZoneId.PEACE) && !allowPeaceAttack())
			{
	...
	public boolean setActiveClass(int classIndex)
	{
		...
		try
		{
			if (EventsInterface.isRegistered(getObjectId()))
			{
				return false;
			}
	...
	{
		if ((Config.Boost_EXP_COMMAND && _addXpSp) || (!Config.Boost_EXP_COMMAND))
		{
	...
	private synchronized void cleanup()
	{
		...
		try
		{
			EventsInterface.onLogout(getObjectId());
			if (EventsInterface.isParticipating(getObjectId()))
			{
				EventsInterface.eventOnLogout(getObjectId());
			}
		}
		catch (Exception e)
		{
	...
	public void rewardAioSkills()
	{
		...
		sendMessage("[Aio System]: ADM concedeu a voce as habilidades de Aio.");
	}
	...
	public final void sendDamageMessage(L2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss)
	{
		...
		if (isInOlympiadMode() && (target instanceof L2PcInstance) && ((L2PcInstance) target).isInOlympiadMode() && ((((L2PcInstance) target).getOlympiadGameId() == getOlympiadGameId()) && !((L2PcInstance) target).isInvul() && !((L2PcInstance) target).inObserverMode()))
		{
	...
	public void setDialogId(DialogId id)
	{
		_currentDialogId = id;
	}
	...
	public void startAdventTask()
	{
		if (_adventBonusTask == null)
		{
			int advent_time = AdventBonus.getInstance().getAdventTime(getObjectId());
			if (advent_time < 14400)
			{
				_adventBonusTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(new AdventPoints(), 30000, 30000);
	...
	public class AdventBlessingEnd implements Runnable
	{
		...
		public void run()
		{
			L2PcInstance.this.stopSpecialEffect(AbnormalEffect.ADVENTURER_BLESSING);
	...
	{
		return AdventBonus.getInstance().getAdventTime(getObjectId());
	}
	
	public void incAdventPoints(int value, boolean decreasetime)
	{
		int adventPoints = AdventBonus.getInstance().getAdventPoints(getObjectId());
		int adventTime = AdventBonus.getInstance().getAdventTime(getObjectId());
		...
		if (decreasetime)
		{
			...
			AdventBonus.getInstance().setAdventTime(getObjectId(), adventTime, true);
		}
		if (_adventBonusTask != null)
		{
			if ((adventPoints + value) >= 7200)
			{
				...
				{
					startSpecialEffect(AbnormalEffect.ADVENTURER_BLESSING);
		...
		AdventBonus.getInstance().setAdventPoints(getObjectId(), adventPoints, true);
	...
	public float getRate(Rates rateType, int itemId, boolean isRaid)
	{
		switch (rateType)
		{
			case PREMIUM_BONUS_EXP:
				return hasPremium() ? PremiumTable.PREMIUM_BONUS_EXP : 1;
			case PREMIUM_BONUS_SP:
				return hasPremium() ? PremiumTable.PREMIUM_BONUS_SP : 1;
			case SPOIL:
			case DROP_ITEM:
				...
				switch (rateType)
				{
					case SPOIL:
						if (hasPremium)
						{
							...
							{
								return Config.PREMIUM_RATE_DROP_ITEMS_ID.get(itemId);
							}
							...
							{
								return Config.PREMIUM_RATE_SPOIL;
							}
						}
					...
					case DROP_ITEM:
						if (hasPremium)
						{
							...
							{
								return isRaid ? Config.PREMIUM_RATE_DROP_ITEMS_BY_RAID : Config.PREMIUM_RATE_DROP_ITEMS;
							}
						}
	...
	private boolean _isRefusingBuff = false;
	
	public void setisRefusingBuff(boolean value)
	{
		_isRefusingBuff = value;
	}
	
	public boolean isRefusingBuff()
	{
		return _isRefusingBuff;
	}
