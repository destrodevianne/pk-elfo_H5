...
import king.server.gameserver.ai.L2FortSiegeGuardAI;
import king.server.gameserver.ai.L2SiegeGuardAI;
import king.server.gameserver.datatables.ChampionData;
import king.server.gameserver.datatables.EventDroplist;
import king.server.gameserver.datatables.EventDroplist.DateDrop;
...
import king.server.gameserver.model.actor.knownlist.AttackableKnownList;
import king.server.gameserver.model.actor.status.AttackableStatus;
...
public class L2Attackable extends L2Npc
{
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character mainDamageDealer)
	{
		...
		if (isChampion() && ChampionData.getInstance().hasPersonalReward(getChampionType()) && (((getLevel() + Config.L2JMOD_CHAMPION_LVL_DIFF) >= player.getLevel()) || (Config.L2JMOD_CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE > 0)))
		{
			for (L2DropData reward : ChampionData.getInstance().getReward(getChampionType()))
			{
				...
				RewardItem item = new RewardItem(reward.getItemId(), champqty);
				
				if ((item != null) && (champqty > 0))
				{
	...
	/**
	 * @return {@code true} if this L2Attackable is champion.
	 */
	@Override
