...
public final class ItemAuctionInstance
{
	...
	public ItemAuctionInstance(final int instanceId, final AtomicInteger auctionIds, final Node node) throws Exception
	{
		...
		_log.log(Level.INFO, getClass().getSimpleName() + ": " + _items.size() + " item(s) e registrado " + _auctions.size() + " auction(s) para " + _instanceId + ".");
	...
	final void checkAndSetCurrentAndNextAuction()
	{
		...
		if ((currentAuction != null) && (currentAuction.getAuctionState() != ItemAuctionState.FINISHED))
		{
			...
			_log.log(Level.INFO, getClass().getSimpleName() + ": Schedule current auction " + currentAuction.getAuctionId() + " para " + _instanceId);
		}
		else
		{
			...
			_log.log(Level.INFO, getClass().getSimpleName() + ": Schedule next auction " + nextAuction.getAuctionId() + " on " + DATE_FORMAT.format(new Date(nextAuction.getStartingTime())) + " para " + _instanceId);
		}
	}
		...
		private final void runImpl() throws Exception
		{
			...
			switch (state)
			{
				case CREATED:
				{
					...
					_log.log(Level.INFO, getClass().getSimpleName() + ": Auction " + _auction.getAuctionId() + " iniciado para " + _auction.getInstanceId());
					checkAndSetCurrentAndNextAuction();
					break;
				}
	...
	final void onAuctionFinished(final ItemAuction auction)
	{
		...
		if (bid != null)
		{
			...
			if (player != null)
			{
				...
				_log.log(Level.INFO, getClass().getSimpleName() + ": Auction " + auction.getAuctionId() + " has finished. Highest bid by " + player.getName() + " para " + _instanceId);
			}
			else
			{
				...
				_log.log(Level.INFO, getClass().getSimpleName() + ": Auction " + auction.getAuctionId() + " has finished. Highest bid by " + CharNameTable.getInstance().getNameById(bid.getPlayerObjId()) + " para " + _instanceId);
			}
