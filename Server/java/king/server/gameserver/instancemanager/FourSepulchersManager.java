...
import king.server.gameserver.model.actor.instance.L2SepulcherMonsterInstance;
import king.server.gameserver.model.actor.instance.L2SepulcherNpcInstance;
...
public class FourSepulchersManager
{
	...
	private void loadMysteriousBox()
	{
			...
			PreparedStatement ps = con.prepareStatement("SELECT id, count, npc_templateid, locx, locy, locz, heading, respawn_delay, key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY id"))
		{
			...
			_log.info(getClass().getSimpleName() + ": " + _mysteriousBoxSpawns.size() + " Mysterious-Box spawns.");
		}
		catch (Exception e)
		{
	...
	private void loadPhysicalMonsters()
	{
			...
			PreparedStatement ps1 = con.prepareStatement("SELECT Distinct key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY key_npc_id"))
		{
			...
			_log.info(getClass().getSimpleName() + ": " + loaded + " Physical type monsters spawns.");
		}
		catch (Exception e)
		{
	...
	private void loadMagicalMonsters()
	{
			...
			PreparedStatement ps1 = con.prepareStatement("SELECT Distinct key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY key_npc_id"))
		{
			...
			_log.info(getClass().getSimpleName() + ": " + loaded + " Magical type monsters spawns.");
		}
		catch (Exception e)
		{
	...
	private void loadDukeMonsters()
	{
			...
			PreparedStatement ps1 = con.prepareStatement("SELECT Distinct key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY key_npc_id"))
		{
			...
			_log.info(getClass().getSimpleName() + ": " + loaded + " Church of duke monsters spawns.");
		}
		catch (Exception e)
		{
	...
	private void loadEmperorsGraveMonsters()
	{
			...
			PreparedStatement ps1 = con.prepareStatement("SELECT Distinct key_npc_id FROM four_sepulchers_spawnlist Where spawntype = ? ORDER BY key_npc_id"))
		{
			...
			_log.info(getClass().getSimpleName() + ": " + loaded + " Emperor's grave NPC spawns.");
		}
		catch (Exception e)
		{
