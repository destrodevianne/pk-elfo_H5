...
public class Zaken extends AbstractNpcAI
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("nighttime") || event.equalsIgnoreCase("daytime") || event.equalsIgnoreCase("daytime83"))
		{
			...
			enterInstance(player, "[034] Zaken-" + event + ".xml", event);
		}
	...
	private void questSupport(L2Npc npc, L2PcInstance player)
	{
		...
		{
			final int _fragment = 21722;
			final QuestState st = player.getQuestState("Q10501_ZakenEmbroideredSoulCloak");
			if ((st != null) && (st.getState() == State.STARTED) && (st.getInt("cond") == 1))
			{
				if (player.getQuestState("Q10501_ZakenEmbroideredSoulCloak").getQuestItemsCount(_fragment) < 19)
				{
					player.getQuestState("Q10501_ZakenEmbroideredSoulCloak").giveItems(_fragment, Rnd.get(1, 2));
					st.playSound("ItemSound.quest_middle");
				}
				else
				{
					player.getQuestState("Q10501_ZakenEmbroideredSoulCloak").giveItems(_fragment, 1);
				}
				if (player.getQuestState("Q10501_ZakenEmbroideredSoulCloak").getQuestItemsCount(_fragment) >= 20)
				{
					player.getQuestState("Q10501_ZakenEmbroideredSoulCloak").set("cond", "2");
				}
			}
		}
	}
