...
public class PailakaInjuredDragon extends Quest
{
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("latana_animation"))
		{
			...
			player.sendPacket(new SpecialCamera(npc.getObjectId(), 600, 0, 0, 1000, 11000, 1, 0, 1, 0));
	...
	public final String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		switch (npc.getNpcId())
		{
			...
			case VARKAS_PROPHET:
				dropHerb(npc, player, HP_HERBS_DROPLIST);
				if ((cond == 5) && st.hasQuestItems(ENCHSPEAR) && !st.hasQuestItems(STAGE2) && (Rnd.get(100) < 15))
				{
					st.set("cond", "6");
					st.giveItems(STAGE2, 1);
					st.playSound("ItemSound.quest_itemget");
			...
			case PROPHET_GUARD:
				dropHerb(npc, player, HP_HERBS_DROPLIST);
				if ((cond == 5) && st.hasQuestItems(ENCHSPEAR) && !st.hasQuestItems(STAGE2) && (Rnd.get(100) < 30))
				{
					st.set("cond", "6");
					st.giveItems(STAGE2, 1);
					st.playSound("ItemSound.quest_itemget");
