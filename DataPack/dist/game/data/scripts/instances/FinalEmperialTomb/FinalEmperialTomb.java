...
public class FinalEmperialTomb extends Quest
{
	...
	private static final int MIN_PLAYERS = Config.FRINTEZZA_MINPLAYERS;
	private static final int MAX_PLAYERS = Config.FRINTEZZA_MAXPLAYERS;
	...
	private void load()
	{
		...
		try
		{
			...
			File file = new File(Config.DATAPACK_ROOT + "/data/zones/spawnZones/final_emperial_tomb.xml");
			if (!file.exists())
			{
	...
	private boolean checkConditions(L2PcInstance player)
	{
		...
		{
			if (channelMember.getLevel() < Config.FRINTEZZA_MINLEVEL)
			{
