 ...
 */
public final class LuckyPigs extends AbstractNpcAI
{
    // NPC
    ...
    //@formatter:off
    private static final int[] TRIGGER_MOBS_LOW = // Enchanted Valley
    {
        20589, 20590, 20591, 20592, 20593, 20594, 20595, 20596, 20597, 20598,
        20599,
    };
    private static final int[] TRIGGER_MOBS_MEDIUM = // Forest of the Dead, Valley of Saints
    {
        21520, 21521, 21522, 21523, 21524, 21525, 21526, 21527, 21528, 21529,
        ...
        21570, 21571, 21572, 21573, 21574, 21575, 21576, 21577, 21578, 21579,
        21580, 21581, 21582, 21583, 21584, 21585, 21586, 21587, 21588, 21589,
        21590, 21591, 21592, 21593, 21594, 21595, 21596, 21597, 21598, 21599,
        21600, 21601,
    };
    private static final int[] TRIGGER_MOBS_TOP = // Beast Farm, Plains of the Lizardmen, Sel Mahum Training Grounds, Field of Silence, Field of Whispers, Crypts of Disgrace, Den of Evil, Primeval Isle, Dragon Valley
    {
        ...
        18903, 18904, 18905, 18906, 18907, 22196, 22197, 22198, 22199, 22200,
        22201, 22202, 22203, 22204, 22205, 22206, 22207, 22208, 22209, 22210,
        22211, 22212, 22213, 22214, 22215, 22216, 22217, 22218, 22219, 22220,
        22221, 22222, 22223, 22224, 22225, 22226, 22227, 22650, 22651, 22652,
        22653, 22654, 22655, 22656, 22657, 22658, 22659, 22691, 22692, 22693,
        22694, 22695, 22696, 22697, 22698, 22699, 22700, 22701, 22702, 22703,
        22704, 22705, 22706, 22707, 22742, 22743, 22744, 22745, 22768, 22769,
        22770, 22771, 22772, 22773, 22774, 22775, 22776, 22777, 22778, 22779,
        22780, 22781, 22782, 22783, 22784, 22785, 22786, 22787, 22788, 22815,
    ...
    // Misc
    private static final int LUCKY_PIG_CHANCE = 1; // 0.1%
    //@formatter:off
    ...
    private LuckyPigs()
    {
        super(LuckyPigs.class.getSimpleName(), "ai/group_template");
            ...
            case "CHECK_FOOD":
            {
                ...
                switch (foodState)
                {
                    case 0: // Looking for item
                    {
                        if (foodItem != null)
                        {
                            _log.warning(LuckyPigs.class.getSimpleName() + ": Lucky pig trying to find another food while currently isn't eaten yet!");
                            break;
                        }
                        ...
                        final L2ItemInstance food = (L2ItemInstance) L2World.getInstance().getVisibleObjects(npc, 300)
                             ...
                             .filter(obj -> (obj.getId() == Inventory.ADENA_ID))
    ...
    public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
    {
        ...
        {
            if ((getRandom(1000) < LUCKY_PIG_CHANCE))
            {
                final int minAdena;
    ...
    private void manageDrop(L2Npc luckyPig, L2PcInstance player, boolean isGold)
    {
        ...
        if (npcId > 0)
        {
            final List<Integer> catData = new FastList<>();
    ...
    private boolean calculateGoldChance(L2Npc luckyPig)
    {
        ...
        int finalChance = LUCKY_PIG_GOLD_BASE_CHANCE[eatCount];
