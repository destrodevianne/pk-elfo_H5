...
import pk.elfo.gameserver.script.EventDrop;
...
public class SavingSanta extends AbstractNpcAI
{
	...
	private static final Map<String, Long> _rewardedPlayers = new FastMap<>();
	private static final Map<String, Long> _blessedPlayers = new FastMap<>();
	...
	// 15th December 2012
	private static GregorianCalendar START_EVENT_CALENDAR = new GregorianCalendar(2012, 11, 15);
	// 6th January 2013
	private static GregorianCalendar END_EVENT_CALENDAR = new GregorianCalendar(2013, 0, 6);
	...
	public SavingSanta(String name, String descr)
	{
		super(name, descr);
	...
	public String onSpellFinished(L2Npc npc, L2PcInstance player, L2Skill skill)
	{
		...
		if (skill.getId() == 6100)
		{
			...
			for (L2PcInstance pl : npc.getKnownList().getKnownPlayersInRadius(600))
			{
				...
				{
					// Oh. I'm bored.
					npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[15]));
				}
				else if ((result == -1) || (result == 2))
				{
					// Now!! Those of you who lost, go away!
					// What a bunch of losers.
					npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[6 + getRandom(2)]));
				...
				else if ((result == 1) || (result == -2))
				{
					...
					if ((level == 1) || (level == 2))
					{
						...
						npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[10 + getRandom(4)]));
					}
					else if (level == 3)
					{
						SkillTable.getInstance().getInfo(23018, 1).getEffects(pl, pl);
						// Stop it, no more... I did it because I was too lonely...
						npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[13]));
					}
					else if (level == 4)
					{
						...
						npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[14 + getRandom(2)]));
						...
						final NpcSay santaSaved = new NpcSay(holidaySled.getObjectId(), 10, holidaySled.getNpcId(), NPC_STRINGS[28]);
						santaSaved.addStringParameter(pl.getName());
						holidaySled.broadcastPacket(santaSaved);
						// Oh ho ho.... Merry Christmas!!
						holidaySled.broadcastPacket(new NpcSay(holidaySled.getObjectId(), 0, holidaySled.getNpcId(), NPC_STRINGS[17]));
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("ThomasCast1"))
		{
			if (!npc.isDecayed())
			{
				...
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[getRandom(6)]));
			}
			else
			{
				if (!_isSantaFree)
		...
		else if (event.equalsIgnoreCase("ThomasCast2"))
		{
			if (!npc.isDecayed())
			{
				...
				npc.doCast(SkillTable.getInstance().getInfo(6100, getRandom(1, 3)));
				// It's hurting... I'm in pain... What can I do for the pain...
				// No... When I lose that one... I'll be in more pain...
				// Hahahah!!! I captured Santa Claus!! There will be no gifts this year!!!
				// Now! Why don't you take up the challenge?
		...
		else if (event.equalsIgnoreCase("SantaRewarding1"))
		{
			...
			npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[21]));
		}
		else if (event.equalsIgnoreCase("SantaRewarding2"))
		{
			startQuestTimer("SantaRewarding3", 5000, npc, player);
			// I have a gift for $s1.
			final NpcSay iHaveAGift = new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[23]);
		...
		else if (event.equalsIgnoreCase("SantaRewarding3"))
		{
			...
			if (_isJackPot)
			{
				...
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[25]));
			...
			else
			{
				...
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[24]));
		...
		else if (player != null)
		{
			...
			else if (event.equalsIgnoreCase("buyHat"))
			{
				...
				player.getInventory().destroyItemByItemId(event, X_MAS_TREE1, 10, player, npc);
				player.getInventory().addItem(event, SANTAS_HAT_ID, 1, player, npc);
			}
			// FIXME: Unhardcore html!
	...
	public String onAggroRangeEnter(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		if (npc.getNpcId() == THOMAS_D_TURKEY_ID)
		{
			// I guess you came to rescue Santa. But you picked the wrong person.
			npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(), NPC_STRINGS[8]));
		}
		return null;
	}
	...
	{
		new SavingSanta(SavingSanta.class.getSimpleName(), "events");
	}
}
