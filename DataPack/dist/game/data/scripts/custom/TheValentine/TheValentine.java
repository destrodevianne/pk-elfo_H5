...
public class TheValentine extends Quest {
	...
	private static final int[] GLOBAL_DROP = { V_DARK_CHOCOLATE, V_WHITE_CHOCOLATE, V_FRESH_CREAM };
	// Minimum and maximum count of dropped items
	private static final int[] GLOBAL_DROP_COUNT = { 1, 1 };
	...
	private static final String[] ON_ENTER_ANNOUNCE = { "Valentine's Event is currently active.\nCollect all items and make your Valentine Cake!" }; 
	// X cords for Queen of Hearts spawn
	private static final int[] X_CORDS = { 	147698, 147443, 82218, 82754, 15064, 111067, 
											-12965, 87362, -81037, 117412, 43983, -45907, 
											12153, -84458, 114750, -45656, -117195 };
	// Y cords for Queen of Hearts spawn
	private static final int[] Y_CORDS = { 	-56025, 26942, 148608, 53573, 143254, 218933, 
											122914, -143166, 150092, 76642, -47758, 49387, 
											16753, 244761, -178692, -113119, 46837 };
	// Z cords for Queen of Hearts spawn
	private static final int[] Z_CORDS = { 	-2775, -2205, -3473, -1496, -2668, -3543, 
											-3117, -1293, -3044, -2695, -797, -3060, 
											-4584, -3730, -820, -240, 367 };

	private static final DateRange EVENT_DATES = DateRange.parse(EVENT_DATE,
			new SimpleDateFormat("dd MM yyyy", Locale.US));

	public TheValentine(final int questId, final String name,
			final String descr) {
		super(questId, name, descr);

		EventDroplist.getInstance().addGlobalDrop(
				GLOBAL_DROP,
				GLOBAL_DROP_COUNT,
				(GLOBAL_DROP_CHANCE * 10000), EVENT_DATES);
		Announcements.getInstance().addEventAnnouncement(EVENT_DATES,
				ON_ENTER_ANNOUNCE);
		...
		if (EVENT_DATES.isWithinRange(currentDate)) {
			for (int i = 0; i < X_CORDS.length; i++) {
				addSpawn(QUEEN_OF_HEARTS, X_CORDS[i], Y_CORDS[i],
						Z_CORDS[i], 0, false, 0);
			}
		}
