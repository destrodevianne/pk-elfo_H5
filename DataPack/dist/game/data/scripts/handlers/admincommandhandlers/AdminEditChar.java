...
import king.server.gameserver.network.serverpackets.GMViewItemList;
...
public class AdminEditChar implements IAdminCommandHandler
{
	...
	private void gatherCharacterInfo(L2PcInstance activeChar, L2PcInstance player, String filename)
	{
		...
		{
			if (ExpirableServicesManager.getInstance().isServiceUnlimited(ServiceType.PREMIUM, player))
			{
				premiumInfo = "Active, unlimited time";
			}
			else if (ExpirableServicesManager.getInstance().isServiceTemporary(ServiceType.PREMIUM, player))
			{
				premiumInfo = "Active until logout";
			}
			else
			{
				premiumInfo = "Active until " + Util.formatDate(new Date(ExpirableServicesManager.getInstance().getExpirationDate(ServiceType.PREMIUM, player)), "dd.MM.yyyy HH:mm");
			}
		}
		else
		{
