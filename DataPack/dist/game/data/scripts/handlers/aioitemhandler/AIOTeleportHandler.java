...
public class AIOTeleportHandler implements IAIOItemHandler
{
	...
	/**
	 * Will try to make the player payment. If success paying, will return true, otherwise, will return false
	 * @param player
	 * @return boolean
	 */
	private boolean paymentDone(L2PcInstance player)
	{
		...
		if ((coin = player.getInventory().getItemByItemId(AIOItem_Config.AIOITEM_GK_COIN)) != null)
		{
			if (coin.getCount() >= AIOItem_Config.AIOITEM_GK_PRICE)
			{
				player.destroyItemByItemId("AIOItem", coin.getItemId(), AIOItem_Config.AIOITEM_GK_PRICE, player, true);
				return true;
			}
