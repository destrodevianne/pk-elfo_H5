...
public class AIOSchemeHandler implements IAIOItemHandler
{
	...
	public void onBypassUse(L2PcInstance player, String command)
	{
		...
		{
			if (player.getProfileNames().size() == AIOItem_Config.AIOITEM_SCHEME_MAX_PROFILES)
			{
	...
	/**
	 * Will return true if the player made the payment for the service, otherwise, will return false
	 * @param player
	 * @param action
	 * @return boolean
	 */
	private boolean paymentDone(L2PcInstance player, Action action)
	{
		...
		if ((coin = player.getInventory().getItemByItemId(AIOItem_Config.AIOITEM_SCHEME_COIN)) == null)
		{
		...
		switch (action)
		{
			case CREATE_PROFILE:
			{
				if (coin.getCount() < AIOItem_Config.AIOITEM_SCHEME_PROFILE_PRICE)
				{
					player.sendMessage("You dont have enought " + coin.getName() + " to create a new scheme!");
					return false;
				}
				player.destroyItemByItemId("AIOItem", coin.getItemId(), AIOItem_Config.AIOITEM_SCHEME_PROFILE_PRICE, player, true);
				return true;
			}
			case GET_BUFFS:
			{
				if (coin.getCount() < AIOItem_Config.AIOITEM_SCHEME_PRICE)
				{
					player.sendMessage("You dont have enought " + coin.getName() + " to create a new scheme!");
					return false;
				}
				player.destroyItemByItemId("AIOItem", coin.getItemId(), AIOItem_Config.AIOITEM_SCHEME_PRICE, player, true);
				return true;
			}
			default:
				return false;
	...
	/**
	 * Will show a window with the buffs from the given category to be added to the player schemes profile
	 * @param category
	 * @param profile
	 * @param player
	 */
